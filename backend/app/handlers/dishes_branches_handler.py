...
def create_new_dish(dish: modelNewDish, db: Session = Depends(get_db)):
    ...
    if dish_alrExists:
        raise HTTPException(
            status_code=400, detail="El platillo ya está registrado con ese nombre.")
    new_dish = Dish(
        name=dish.name,
    ...
    for i, ingredient_id in enumerate(dish.ingredients):
        ...
        if not ingredient:
            raise HTTPException(
                status_code=400, detail=f"Ingrediente con ID {ingredient_id} no existe.")
    ...
    for i, branch_id in enumerate(dish.branches):
        ...
        if not branch:
            raise HTTPException(
                status_code=400, detail=f"Sucursal con ID {branch_id} no existe.")
...
def deleteDish(id_dish: int, db: Session = Depends(get_db)):
    ...
    if not dish_should_exist:
        raise HTTPException(
            status_code=400, detail="El platillo a eliminar no existe.")
...
def editDish(editable_dish: modelExistentDish, db: Session = Depends(get_db)):
    dish_should_exist = db.query(Dish).filter(
        Dish.id == editable_dish.id).first()
    if not dish_should_exist:
        raise HTTPException(
    ...
    db.query(BranchMenu).filter(BranchMenu.id_dish ==
                                editable_dish.id).delete(synchronize_session=False)
